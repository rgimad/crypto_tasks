...
static void get_random_n_bits(mpz_t r, size_t bits) {
...
static void get_random_n(mpz_t r, mpz_t max) {
	//int cnt = 0;
	do {
...
elgamal_ctx_t elgamal_create_context() {
    ...
    elgamal_ctx_t ctx;
    mpz_init_set(ctx.pub_key.p, p);
    mpz_init_set(ctx.pub_key.g, g);
    mpz_init_set(ctx.pub_key.y, y);
    mpz_init_set(ctx.pub_key.q, q);
    mpz_init_set(ctx.priv_key, x);
    return ctx;
}
...
elgamal_sgn_t elgamal_sign_msg(elgamal_ctx_t ctx, char *message) {
	...
	mpz_t r; mpz_init(r);
	mpz_powm(r, ctx.pub_key.g, k, ctx.pub_key.p);
	//gmp_printf("r = %Zd\n", r);
	//printf("r length is %lu bits\n", mpz_sizeinbase(r, 2));

	mpz_t ro; mpz_init(ro);
	mpz_mod(ro, r, ctx.pub_key.q);
	//gmp_printf("ro = %Zd\n", ro);
	...
	mpz_mul(s, ro, h);
	mpz_mul(s, s, k);
	mpz_sub(s, s, ctx.priv_key);
	mpz_mod(s, s, ctx.pub_key.q);
	//gmp_printf("s = %Zd\n", s);
	//printf("s length is %lu bits\n", mpz_sizeinbase(s, 2));

    elgamal_sgn_t sgn;
    mpz_init_set(sgn.r, r);
    mpz_init_set(sgn.s, s);
    return sgn;
}
...
int elgamal_sign_check(elgamal_sgn_t sgn, elgamal_pub_t pub_key, char *message) {
    ...
    mpz_t ro; mpz_init(ro);
	mpz_mod(ro, sgn.r, pub_key.q);
    mpz_t tmp; mpz_init(tmp);
    
    /* Checking: (using equation: r^(ro*h) mod p == (g^s)*y mod p */
	mpz_t left; mpz_init(left);
	mpz_mul(tmp, ro, h);
	mpz_powm(left, sgn.r, tmp, pub_key.p);
	gmp_printf("left = %Zd\n", left);

	mpz_t right; mpz_init(right);
	mpz_powm(right, pub_key.g, sgn.s, pub_key.p);
	mpz_mod(tmp, pub_key.y, pub_key.p);
	mpz_mul(right, right, tmp);
	mpz_mod(right, right, pub_key.p);
	gmp_printf("righ = %Zd\n", right);
